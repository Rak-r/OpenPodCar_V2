...
path_to_yaml = PathJoinSubstitution([FindPackageShare("pod2_sensor_tools"), "config", "realsense_params.yaml"])
    ...
    if (os.getenv('ROS_DISTRO') == "dashing") or (os.getenv('ROS_DISTRO') == "eloquent"):
        ...
        return [
            launch_ros.actions.Node(
                ...
                remappings=[('depth/image_rect_raw', '/depth'),
                            ('/camera/depth/camera_info', '/depth_camera_info'),
                            ('/camera/depth/color/points', '/cloud_in')],
                parameters=[set_configurable_parameters(configurable_parameters)
                            , params_from_file
                            ],
                output='screen',
